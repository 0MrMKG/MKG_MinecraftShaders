...
vec3 waterColorArr[24] = {
    ...
    vec3(0.02, 0.2, 0.27),      // 13000 - 14000 夜晚
    vec3(0.02, 0.2, 0.27),      // 14000 - 15000
    vec3(0.02, 0.2, 0.27),      // 15000 - 16000
    vec3(0.02, 0.2, 0.27),      // 16000 - 17000
    vec3(0.02, 0.2, 0.27),      // 17000 - 18000
...
// 009-04 光线追踪
vec3 RayTracing(vec3 color, vec4 positionInViewCoord,vec3 reflectDirection){

vec3 Point = positionInViewCoord.xyz;
const int reflection = 40;
for(int i=0; i<reflection; i++)
{
    Point = Point + 0.2*reflectDirection;
    ...
    return vec3(0);
     ...
     float depth = texture2D(depthtex0, positionInScreenCoord.st).x; // 深度
        ...
        if(depth<=positionInScreenCoord.z || i==reflection-1) {
            return texture2D(texture, positionInScreenCoord.st).rgb*0.9;
        }
}
return vec3(0);
}
...
vec3 drawWater(vec3 color, vec4 positionInWorldCoord, vec4 positionInViewCoord, vec3 normal) {
   ...
   //加入抖动量
    ...
    finalColor= mix(RayTracing(color,positionInViewCoord,reflectDirection), finalColor,0.6);
