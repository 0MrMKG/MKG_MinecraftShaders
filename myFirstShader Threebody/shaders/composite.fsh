...
vec3 drawSky(vec3 color, vec4 positionInViewCoord, vec4 positionInWorldCoord) {
    ...
    vec3 mySunColor = vec3(6,0.1,0);
    vec3 mySunColor_2 = vec3(10,10,10);
    vec3 mySunColor_3 = vec3(0,0,15);
    vec3 mySkyColor = vec3(2,0.1,0);

    //第二个太阳坐标

    vec3 Sun_2_Position = sunPosition + vec3(20,20,0);
    vec3 Sun_3_Position = Sun_2_Position + vec3(20,20,0);
    ...
    float disToSun_2 = 1.0 - dot(normalize(positionInViewCoord.xyz), normalize(Sun_2_Position));
    float disToSun_3 = 1.0 - dot(normalize(positionInViewCoord.xyz), normalize(Sun_3_Position));
    ...
    // 03绘制圆形太阳2号
    vec3 drawSun_2 = vec3(0);
    if (dis>0.99999&&disToSun_2<0.02)
    drawSun_2 = mySunColor_2 * 2 * (1.0-isNight);

    // 04绘制圆形太阳3号
    vec3 drawSun_3 = vec3(0);
    if (dis>0.99999&&disToSun_3<0.03)
    drawSun_3 = mySunColor_3 * 2 * (1.0-isNight);
    ...
    // 03雾和第二个太阳颜色混合
//    float Sun_2_MixFactor = clamp(1.0 - disToSun_2, 0, 1) * (1.0-isNight);
//    finalColor = mix(finalColor, mySunColor, pow(Sun_2_MixFactor, 4));

    float Sun_3_MixFactor = clamp(1.0, 0, 1) * (1.0-isNight);
    finalColor = mix(finalColor, mySunColor, pow(Sun_3_MixFactor, 4));

    // 根据距离进行最终颜色的混合
    return mix(color, finalColor, clamp(pow(dis, 3), 0, 1)) + drawSun + drawMoon + drawSun_2 + drawSun_3 ;
}
...
//    vec3 mySkyColor = mix(skyColorArr[hour], skyColorArr[next], delta);
    ...
    vec4 orange = vec4(1,0.5,0.3,1);
    gl_FragData[0] = color*orange; // 基色
    gl_FragData[1] = getBloomSource(color)*orange; // 传递高光原图
}