...
float id = texture2D(process_blur_2, texcoord.st).x; //检测物体的id用于光照
...
vec4 alter_bloom_color(vec4 bloom)
{
...
//    color = GetBloomColor(color);
    ...
    gl_FragData[0] = color;
    vec4 bloom = GetBloomColor(color);
    bloom = alter_bloom_color(bloom);
    gl_FragData[1]= bloom;
}
